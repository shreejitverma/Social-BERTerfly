...
def predict_tweet(username):
    tweet_path = f"twitter_data/tweets_{str(username)}.csv"
    ...
    op_json = {"name": str(username), "type": str(per_op)}
    et = np.sum(tweet_vals,axis = 0)
    summer = np.sum(et)
    intro = np.sum(et[8:])/np.sum(et)*100
    intui = np.sum(et[:4] + et[8:12]) / summer * 100
    feeli = np.sum(et[:2] + et[4:6] + et[8:10] + et[12:14]) / summer * 100
    ...
    with open("static/results.js","w") as perfile:
        perstr = f"var personality_data={op_json}" + "\n"
        perfile.write(perstr)
    predict_follow(username)
    return op_json

def predict_follow(username):
    follow_df = pd.read_csv(f"twitter_data/fol_{str(username)}.csv")
    follow_json = {}
    follow_ids = {}
    for i in range(5):
        list_j = follow_df.tweets.iloc[i].split(", \'")
        new_list = [clean_text(j) for j in list_j]
        ...
        with open("static/results2.js","w") as perfile:
            perstr = f"var follower_data={follow_json}" + "\n"
            folstr = f"var follower_ids={follow_ids}" + "\n"
            perfile.write(perstr)
